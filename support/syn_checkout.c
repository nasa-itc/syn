...
*/
uart_info_t SynUart;
SYN_Device_HK_tlm_t SynHK;
SYN_Device_Data_tlm_t SynData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = SYN_CommandDevice(&SynUart, SYN_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = SYN_RequestHK(&SynUart, &SynHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("SYN_RequestHK command success\n");
                }
                else
                {
                    OS_printf("SYN_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_SYN:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = SYN_RequestData(&SynUart, &SynData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("SYN_RequestData command success\n");
                }
                else
                {
                    OS_printf("SYN_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = SYN_CommandDevice(&SynUart, SYN_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    SynUart.deviceString = SYN_CFG_STRING;
    SynUart.handle = SYN_CFG_HANDLE;
    SynUart.isOpen = PORT_CLOSED;
    SynUart.baud = SYN_CFG_BAUDRATE_HZ;
    status = uart_init_port(&SynUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", SynUart.deviceString, SynUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", SynUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&SynUart);

    OS_printf("Cleanly exiting syn application...\n\n"); 
    return 1;
}
